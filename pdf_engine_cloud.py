...
class PDF(FPDF):
    ...
    def inserir_figura(self, fig, largura=180, x=None, y=None):
        """
        Salva uma figura matplotlib temporariamente e insere no PDF.
        """
        if fig is None:
            return
...
def tabela_top10(df, coluna_valor, coluna_categoria):
    ...
    for categoria, valor in top.items():
        largura = int((valor / max_val) * 30)  # barra proporcional
        barra = "█" * largura
...
def tabela_correlacao(df, colunas_numericas):
    ...
    return linhas[:15]  # top 15 correlações


# ---------- INSERÇÃO DE TABELAS NO PDF ----------

def pdf_tabela(pdf, colunas, linhas, largura_total=190, altura_linha=6):
    """
    Renderiza uma tabela simples no PDF.
    """
